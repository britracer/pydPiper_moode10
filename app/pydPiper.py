...
import threading, logging, queue, time, sys, getopt, moment, signal, os, copy, requests, subprocess
from . import displays
from . import sources
from . import pause
from . import pydPiper_config
...
class music_controller(threading.Thread):
    ...
    def checkaccuweatherreturn(self, status_code, querystr=""):
        if status_code == 400:
    ...
    def updateforecast(self):
        ...
        while not exitapp[0]:
            ...
            if pydPiper_config.WEATHER_SERVICE == 'accuweather':
                ...
                if self.checkaccuweatherreturn(r.status_code, querystr):
                    try:
                        res = r.json()
    ...
    def updatesystemvars(self):
        ...
        while not exitapp[0]:
            ...
            current_ip = subprocess.getoutput(u"ip -4 route get 1 | head -1 | cut -d' ' -f8 | tr -d '\n'").strip()
